# Churn Warning — Hệ thống Dự đoán Rời bỏ Khách hàng
...
Hệ thống biến dữ liệu khách hàng và giao dịch hàng tháng của VNPost thành danh sách rủi ro churn phục vụ CSKH:
- Score khách hàng active trên horizon 2 tháng.
- Xuất high-risk list vào PostgreSQL, có thể chuyển thành CSV.
- Gắn xác suất rủi ro và reason signals; ưu tiên SHAP khi có artifact, fallback bằng rule/feature-importance khi thiếu artifact.
- Theo dõi chất lượng mô hình bằng run log, score drift, PSI/KS và backtest precision-in-list.
...
run_churn_pipeline -> weekly 05:00 model run
```

`run_churn_pipeline` là DAG model weekly hiện tại, mặc định chạy thứ Bảy 05:00:

```bash
python -m pipelines.churn.cli
```

| Stage | Vai trò | Output |
|---|---|---|
| ZIP ingestion | Unzip, validate header/schema, transform, load PostgreSQL, ghi `ingest.ingest_log` | Raw tables trong `public` |
| Feature generation | Tạo `cus_lifetime` static, `cus_lifetime_{W}m_*` theo window, và `cus_feature_{W}m_*` | `data_static`, `data_window` |
| Dataset prep | Time-aware scope, label horizon, walk-forward W*, prototype, PU weights | Train/eval/predict sets |
| XGBoost training | Train, evaluate, guardrail, accept/reject bundle | `model_bundles/latest` |
| Monthly scoring | Score active customers bằng accepted bundle tốt nhất | Probability + churn flag |
| Risk export | Ghi high-risk list vào PostgreSQL | `data_static.churn_risk_predictions` |
| Monitoring | Run log, score drift, feature drift PSI/KS, backtest | `ml_monitor.*` |

## Feature Engine

Mặc định feature generation dùng PostgreSQL SQL để giữ kết quả ổn định. Có thể bật PySpark cho phần lifetime-as-of khi dữ liệu lớn:

```env
FEATURE_ENGINE=pyspark
BUILD_LIFETIME_ASOF=true
```

PySpark đọc `public.cas_customer` qua JDBC và ghi `data_window.cus_lifetime_{W}m_{start}_{end}`. Sliding-window SQL vẫn chạy trong PostgreSQL để bảo toàn logic feature hiện tại.
...
crm_churn_warning/
├── dags/                           # Airflow DAGs
│   ├── ingest_data.py          #   Scheduled: scan ZIP → validate
│   ├── build_features.py        #   Triggered: feature generation
│   ├── generate_eda_reports.py  #   Triggered: EDA report generation
│   ├── run_churn_pipeline.py        #   Weekly 05:00 model/scoring/export
│   └── clean_runtime_files.py    #   Scheduled: cleanup old bundles
│
...
│   └── api/                        #   Batch output contract + access examples
│
...
flowchart TD
    ...
    Ingest["DAG: ingest_data - Scan và Load ZIP"]
    Files --> Ingest

    Features["DAG: build_features - Sliding window"]
    Ingest --> Features

    Pipeline["DAG: run_churn_pipeline - weekly 05:00 model run"]
    ...
    Housekeeping(["DAG: clean_runtime_files - Hàng ngày 03:00"])
    Export -. "chạy độc lập theo cron" .-> Housekeeping
```

**Tóm tắt luồng:**
1. **Trigger tự động**: ZIP files được drop vào `incoming/` → DAG `ingest_data` quét ngày 13 & 23 hàng tháng lúc 09:00.
2. **Chain reaction**: Ingest thành công → tự động trigger `build_features`.
3. **Weekly model**: `run_churn_pipeline` chạy hằng tuần lúc 05:00 theo `CHURN_MODEL_SCHEDULE`, đọc feature mới nhất để train/score/export.
4. **Output**: Bảng `data_static.churn_risk_predictions` được TRUNCATE và ghi mới mỗi lần chạy — luôn là snapshot mới nhất.
5. **CSKH consume**: Bộ phận CSKH query trực tiếp từ PostgreSQL hoặc export CSV từ bảng risk.
...
|-----|----------|---------|---------|--------|
| `ingest_data` | `0 9 13,23 * *` | Cron | 2 | Scan ZIP → Load DB → Validate → trigger features |
| `build_features` | None | Triggered by ingest | 1 | Sliding window feature generation |
| `run_churn_pipeline` | `0 5 * * 6` | Cron weekly | 0 | Full 8-step pipeline (dataset prep + model + score + export) |
| `clean_runtime_files` | `0 3 * * *` | Cron (hàng ngày) | 0 | Dọn bundles cũ, logs, saved/failed data |
...
| `data_window` | `cus_lifetime_{W}m_{start}_{end}` | Lifetime metrics tính đến cuối từng window, không dùng dữ liệu tương lai | Feature gen |
...
| `metric_f2_val` | F0.5 score used for accept/reject |
| `metric_roc_auc_val` | PR-AUC score for this pipeline |
| `is_accepted` | Model có được chấp nhận không |
| `accept_rule` | Lý do accept/reject |
| `prev_accepted_f2` | F0.5 của model trước |
...
```bash
# Clone repository
git clone <repo-url> crm_churn_warning
cd crm_churn_warning
...
|-----------|----------|-----------|
| Disk khả dụng < 15% | Log/cache tích tụ | Trigger `clean_runtime_files` thủ công |
...
DAG `clean_runtime_files` chạy **hàng ngày lúc 03:00** với chính sách giữ dữ liệu:
...
| **Project Structure** | [`docs/architecture/project_structure.md`](docs/architecture/project_structure.md) | Layout repo, runtime boundaries, schedule |
...
| **Batch Output Contract** | [`docs/api/api_spec.yaml`](docs/api/api_spec.yaml) | Documents that no realtime API exists yet; output is PostgreSQL batch tables |
