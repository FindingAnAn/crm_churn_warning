...
```text
ds_churn_ingest -> ds_churn_features
ds_churn_pipeline -> weekly 05:00 model run
```
...
│   ├── ds_churn_pipeline.py        #   Weekly 05:00 model/scoring/export
...
flowchart TD
    ...
    Pipeline["DAG: ds_churn_pipeline - weekly 05:00 model run"]
...
2. **Chain reaction**: Ingest thành công → tự động trigger `ds_churn_features`.
3. **Weekly model**: `ds_churn_pipeline` chạy hằng tuần lúc 05:00 theo `CHURN_MODEL_SCHEDULE`, đọc feature mới nhất để train/score/export.
...
| `ds_churn_features` | None | Triggered by ingest | 1 | Sliding window feature generation |
| `ds_churn_pipeline` | `0 5 * * 6` | Cron weekly | 0 | Full 8-step pipeline (dataset prep + model + score + export) |
