...
def _truncate_or_delete_in_batches(engine, table_name: str, batch_size: int = 50000) -> None:
    ...
    try:
        ...
        logger.info("  [OK] Truncated data_static.cus_lifetime")
        return
    ...
    while True:
        ...
        logger.info(f"  [*] Deleted {deleted_total:,} rows from data_static.cus_lifetime...")

    logger.info(f"  [OK] Cleared data_static.cus_lifetime with batched DELETE ({deleted_total:,} rows)")
