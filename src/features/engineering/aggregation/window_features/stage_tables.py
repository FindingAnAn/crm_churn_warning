"""Helpers for temporary stage tables used by window feature generation."""
...
from src.features.engineering.libs.db_utils import build_bccp_src
from src.features.engineering.libs.db_utils import create_bccp_indexes
...
def _drop_stage_tables(engine: Engine) -> None:
...
def _ensure_source_indexes(engine: Engine) -> None:
    with engine.begin() as conn:
        statements = [
...
def prepare_stage_tables(engine: Engine, start_date: str, end_date: str) -> None:
...
def cleanup_stage_tables(engine: Engine) -> None:
    _drop_stage_tables(engine)
