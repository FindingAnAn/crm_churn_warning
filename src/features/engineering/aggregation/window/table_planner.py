"""Planning helpers for existing and recomputation window tables."""
...
def split_tables_to_keep_and_recompute(existing_tables: Sequence[str], recompute_last_n: int) -> tuple[list[str], list[str]]:
...
def truncate_tables(engine: Engine, table_names: Sequence[str], logger) -> int:
    if not table_names:
        return 0
...
def list_empty_tables(engine: Engine, schema_name: str, table_names: Sequence[str], logger) -> list[str]:
    if not table_names:
        return []
...
def list_existing_window_tables(engine: Engine) -> list[str]:
...
def parse_window_size(table_name: str) -> int | None:
